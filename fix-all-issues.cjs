  ...
  try {
    ...
    if (fs.existsSync(authDir)) {
      ...
      try {
        ...
        console.log('   ⚠️  تعذر حذف المجلد (قد يكون مستخدماً)');
      }
    } else {
...
async function checkFacebookToken() {
  ...
  try {
    ...
    if (data.error) {
      ...
      console.log('   📝 استخدم: node fix-facebook-token.cjs');
    } else {
...
function showPostFixInstructions() {
  ...
  console.log('4. إذا كان Facebook Token لا يعمل، استخدم: node fix-facebook-token.cjs');
  console.log('\n🔗 روابط مفيدة:');
