...
function startServer() {
  ...
  currentProcess = spawn('node', [
    '--max-old-space-size=256', // حد أقصى 256MB للذاكرة
    '--gc-interval=100',        // garbage collection كل 100ms
    '--expose-gc',              // تفعيل garbage collection يدوياً
    '-r', 'tsx/cjs',           // تحميل tsx
    'src/api/server-mysql.ts'   // ملف الخادم
  ], {
    stdio: 'inherit',
    cwd: process.cwd(),
    env: {
